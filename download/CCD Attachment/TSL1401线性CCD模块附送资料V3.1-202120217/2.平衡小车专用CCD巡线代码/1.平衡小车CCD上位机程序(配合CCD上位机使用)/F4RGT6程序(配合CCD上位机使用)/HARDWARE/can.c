...
void CAN1_RX0_IRQHandler(void)
{
  u8 i;
	...
	if(CAN_ON_Flag==0)
	{
		//Verify the first set of data. The first 7 bits of the first set of data must be equal to ON_rxbuf[]
		//校验第一组数据，第一组数据的前7位必须与ON_rxbuf[]相等
		for(i=0;i<=7;i++) 
		{
			if(i==7)
			{
				//CAN controls the flag position 1. Bit 8 of the first set of data cannot be 0
				//CAN控制标志位置1，第一组数据的第8位不能为0
				CAN_ON_Flag=temp_rxbuf[i]; 
				if(CAN_ON_Flag==1) 
				 {
					 PS2_ON_Flag=0,APP_ON_Flag=0,Remote_ON_Flag=0,Usart1_ON_Flag=0,Usart3_ON_Flag=0;
					 for(i=0;i<7;i++)
					 {
						 temp_rxbuf[i]=0;
					 }
	...
	//If the CAN control mode is enabled, the current processing of data is performed directly
	//如果开启了CAN控制模式，直接对数据进行当前处理
  if(CAN_ON_Flag==1) 
	{
		//Calculate the three-axis target velocity, unit: mm/s
		//计算三轴目标速度，单位：mm/s
		Move_X=(short)((temp_rxbuf[0]<<8)+(temp_rxbuf[1])); 
		Move_Y=(short)((temp_rxbuf[2]<<8)+(temp_rxbuf[3]));
		Move_Z=(short)((temp_rxbuf[4]<<8)+(temp_rxbuf[5]));
		
		//单位转换 Unit conversion mm/s->m/s
		Move_X=Move_X/1000; 
		Move_Y=Move_Y/1000; 
		Move_Z=Move_Z/1000; 
	}
