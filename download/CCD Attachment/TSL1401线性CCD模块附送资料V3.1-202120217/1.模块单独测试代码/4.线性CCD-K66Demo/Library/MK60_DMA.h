...
void DMA_PORTx2BUFF_Init(DMA_CHn CHn,
                         ...
                         DMA_BYTEn byten,
                         uint32_t count,
                         DMA_PORTx2BUFF_cfg cfg);//DMA初始化，读取端口数据到内存


void DMATransDataStart(uint8_t CHn,uint32_t address);//设置DMA传输的目的地址

void DMA_Count_Init(DMA_CHn CHn, PTXn_e ptxn, u32 count, DMA_PORTx2BUFF_cfg cfg);//功能说明：DMA累加计数初始化


u32  DMA_Count_Get(DMA_CHn CHn);//获取累加计数值
