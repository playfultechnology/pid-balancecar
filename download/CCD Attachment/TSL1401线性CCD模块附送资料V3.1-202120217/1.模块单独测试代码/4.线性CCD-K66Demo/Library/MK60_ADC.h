...
void ADC_Init(ADC_Type * adc_n);//初始化ADC模块

void ADC_Start(ADC_Type * adc_n, ADCn_Ch_e adc_ch, ADC_nbit bit);//启动ADC转换

uint16_t ADC_Once(ADC_Type * adc_n, ADCn_Ch_e adc_ch, ADC_nbit bit);//启动ADC转换 获取一次ADC转换值

uint16_t ADC_Mid(ADC_Type * adc_n,  ADCn_Ch_e adc_ch, ADC_nbit bit);//进行ADC3次转换取中间值

uint16_t ADC_Ave(ADC_Type * adc_n,  ADCn_Ch_e adc_ch, ADC_nbit bit, uint16_t N);//进行ADC N次转换取平均值
