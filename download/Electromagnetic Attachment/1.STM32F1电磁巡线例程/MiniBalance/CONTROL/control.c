...
{
	    u8 temp;
			if(voltage<1110||Flag_Stop==1)//电池电压过低关闭电机
			{	                                                
      temp=1;      
      PWMA=0;
      PWMB=0;
...
void Get_RC(u8 mode)
{
	...
	int LX,LY,RX;	//PS2手柄控制变量
	int Yuzhi=20;	//PS2控制防抖阈值
	float step=0.5; //设置速度控制步进值。
	static float Bias,Last_Bias;  //偏差和历史值
	u8 Flag_Move=1;
	if(mode==0)//速度模式
	{
		if(Flag_Way==0)//APP模式下
		{
			if(CAN_ON_Flag==0&&Usart_ON_Flag==0) //CAN/USART未使能，接收蓝牙指令
			{
		...
		else if(Flag_Way==2)//CCD巡线
		{
	   	 ...
	   	 Move_Y=RC_Velocity;
			 ...
			 Move_Z=-Bias*4-(Bias-Last_Bias)*25; //PD控制
	...
	if(IR1==0||IR2==0) Move_X=0,Move_Y=0,Move_Z=0;//如果有检测到障碍，停止
