...
void DMA_PORTx2BUFF_Init(DMA_CHn CHn,void *SADDR,void *DADDR,PTXn_e ptxn,DMA_BYTEn byten,uint32_t count,DMA_PORTx2BUFF_cfg cfg);//DMA初始化，读取端口数据到内存

void DMATransDataStart(uint8_t CHn,uint32_t address);//设置DMA传输的目的地址

void DMA_Count_Init(DMA_CHn CHn, PTXn_e ptxn, u32 count, DMA_PORTx2BUFF_cfg cfg);//DMA累加计数初始化

u32  DMA_Count_Get(DMA_CHn CHn);//返回累加计数值

void DMA_Count_Reset(DMA_CHn CHn);//清除累加计数值
