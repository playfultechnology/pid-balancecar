...
/*********************************************************************************
  *
  * 函数功能：初始化ADC模式
  * 入口参数：ADCn_e： 要初始化的ADC模块， ADC0  ADC1
  * 返 回 值：无
...
/*********************************************************************************
  *
  * 函数功能：启动ADC转换
  * 入口参数：adc_n ：  模块名ADC0或ADC1 
  * 入口参数：bit   ：  精度选择ADC_8bit，ADC_12bit，ADC_10bit，ADC_16bit
  * 返 回 值：无
...
/*********************************************************************************
  *
  * 函数功能：启动ADC转换 获取一次ADC转换值
  * 入口参数：adc_n ：  模块名ADC0或ADC1 
  * 入口参数：adc_ch：  ADC通道编号 
  * 入口参数：bit   ：  精度选择ADC_8bit，ADC_12bit，ADC_10bit，ADC_16bit
  * 返 回 值：result：  ADC转换结果
...
/*********************************************************************************
  ...
  *
  ********************************************************************************/
uint16_t ADC_Mid(ADCn_e adc_n,ADCn_Ch_e adc_ch,ADC_nbit bit) 
{
    uint16_t i,j,k,tmp;
    //1.取3次A/D转换结果
...
/*********************************************************************************
  ...
  * 入口参数：adc_ch：  ADC通道编号 
  * 入口参数：bit   ：  精度选择ADC_8bit，ADC_12bit，ADC_10bit，ADC_16bit
  * 入口参数：N     ：  均值滤波次数（1~65535）
  * 返 回 值：tmp   ：  平均值
  * 实    例：ADC_Ave(ADC0,ADC1_SE8,ADC_12bit,100) 通道编号为 ADC1_SE8的引脚，进行100次ADC转换，计算出平均值返回。 
  *
  ********************************************************************************/
uint16_t ADC_Ave(ADCn_e adc_n,ADCn_Ch_e adc_ch,ADC_nbit bit,uint16_t N)
{
    uint32_t tmp = 0;
    uint8_t  i;
